...
import { useLatestResults, useRunResults } from '@/lib/queries'
...
export default function RunsPage() {
  ...
  // Always ask the backend for the current completed result when /runs opens.
  // This avoids showing a stale empty state after a run has just finished.
  useEffect(() => {
    void latest.refetch()
  }, [latest.refetch])
  ...
  const result = selectedId ? selected.data : undefined
  const runList = latest.data?.run_id ? [latest.data] : []
  ...
  return <main className="runs-page dashboard-page">
    ...
    <section className="dashboard-shell">
      <aside className="dashboard-sidebar glass">
        <div className="sidebar-heading"><div><span className="eyebrow">WORKSPACE</span><h2>Runs</h2></div><button className="icon-btn" type="button" aria-label="Refresh runs" onClick={() => void latest.refetch()}><RefreshCw size={15} /></button></div>
        ...
        {latest.isPending ? <p className="muted">Loading persisted runs…</p> : latest.error ? <p className="run-message" role="alert">{latest.error.message}</p> : runList.length ? <div className="run-history-list">{runList.map((run) => { const key = run.run_id ?? ''; const active = false; return <button type="button" className={`run-history-item ${selectedId === key ? 'selected' : ''}`} key={key} disabled={!key} onClick={() => key && setSelectedId(key)}><span className={`run-history-status ${active ? 'active' : ''}`}><i /> {active ? run.status : 'completed'}</span><b>{formatDate(run.timestamp || run.run_started_at)}</b><small>{run.run_id}</small><span>{summaryLabel(run)}</span></button> })}</div> : <div className="empty"><span>NO RUNS YET</span><small>Start a reconciliation to create the first durable result.</small></div>}
      ...
      <div className="dashboard-main">
        <div className="dashboard-heading"><div><span className="eyebrow">CONTROL ROOM / OVERVIEW</span><h1>Understand every run<br /><em>at a glance.</em></h1><p>Review throughput, outcomes, and the cases that need attention—without losing the evidence behind each decision.</p></div><div className="dashboard-date"><span>SELECTED RUN</span><b>{selectedId ? (result?.run_id || selectedId) : 'No run selected'}</b></div></div>
        {result ? <>
        ...
        </> : <div className="dashboard-empty glass"><CircleAlert size={22} /><h2>No run selected</h2><p>Choose a run from the history panel to inspect its results.</p><button className="primary-btn" onClick={start} disabled={running || uploading} type="button"><Play size={15} /> Start a new run</button></div>}
      </div>
    </section>
  </main>
}
