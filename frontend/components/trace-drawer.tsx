...
export function TraceDrawer({ recordId = '', open: controlledOpen, onOpenChange }: Props) {
  ...
  const trigger = id ? <button ref={triggerRef} className="trace-open" onMouseEnter={() => prefetch(id)} onFocus={() => prefetch(id)} onClick={() => setOpen(true)} aria-haspopup="dialog"><FileSearch data-icon="inline-start" /> Open case trace</button> : null
