...
export function TraceDrawer({ recordId = '', open: controlledOpen, onOpenChange }: Props) {
  ...
  return <><span className="trace-trigger">{trigger}</span><AnimatePresence>{open && <motion.div className="trace-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false) }} initial={reduced ? false : { opacity: 0 }} animate={{ opacity: 1 }} exit={reduced ? undefined : { opacity: 0 }}><motion.aside className="trace-drawer liquid-focal" role="dialog" aria-modal="true" aria-labelledby="trace-title" initial={reduced ? false : { x: '100%' }} animate={{ x: 0 }} exit={reduced ? undefined : { x: '100%' }}><button ref={closeRef} className="trace-close" onClick={() => setOpen(false)} aria-label="Close reasoning trace"><X /></button><span className="eyebrow">TRACE / RECORD ID</span><h2 id="trace-title">{id || 'Reasoning trace'}</h2><label htmlFor="record-id">Backend record ID</label><input id="record-id" value={localRecordId} onChange={(event) => setLocalRecordId(event.target.value)} placeholder="Paste a record id" autoComplete="off"/><p className="trace-hint">Structured evidence returned by the FastAPI reasoning-trace endpoint.</p>{trace.isFetching && <p aria-live="polite"><LoaderCircle className="spin" /> Loading trace…</p>}{trace.error && <div className="run-message" role="alert">{trace.error.message}<button className="trace-open" onClick={() => trace.refetch()}><RefreshCw /> Retry</button></div>}{data && <div className="trace-evidence"><EvidenceBlock title="RECORD IDENTITY" value={{ record_id: data.record_id, provider: data.provider, handled_by_key: data.handled_by_key, final_status: data.final_status }} /><EvidenceBlock title="PROPOSER" value={data.history} /><EvidenceBlock title="VERIFIER" value={asRecord(data.final_decision)?.verifier} /><EvidenceBlock title="FINAL DECISION" value={data.final_decision} /><EvidenceBlock title="TIMING" value={{ wall_clock_time_sec: data.wall_clock_time_sec, active_processing_time_sec: data.active_processing_time_sec, reactive_throttle_wait_sec: data.reactive_throttle_wait_sec, self_paced_wait_sec: data.self_paced_wait_sec, other_pacing_wait_sec: data.other_pacing_wait_sec }} /></div>}</motion.aside></motion.div>}</AnimatePresence></>
}
